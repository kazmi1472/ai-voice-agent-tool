...
class OpenAIClient:
    def __init__(self) -> None:
        ...
        self.simulated = api_key is None or len(api_key.strip()) == 0
        self.client = AsyncOpenAI(api_key=api_key) if not self.simulated else None
    ...
    async def decide_next_action(self, context: Dict[str, Any], last_driver_utterance: str) -> Dict[str, Any]:
        ...
        messages = [
            {"role": "system", "content": REALTIME_SYSTEM_PROMPT.format(**context, last_driver_utterance=last_driver_utterance)},
        ...
        chat = await self.client.chat.completions.create(model="gpt-4o-mini", messages=messages, response_format={"type": "json_object"})
